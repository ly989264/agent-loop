...
the operator's git credentials from inside a round.  ``VALKEY_REAL_`` and
``VSLAB_M2_`` are kept because a consumer's own run-authorising variables must not
leak either; the list is data and a consumer-specific prefix belongs here.
"""
